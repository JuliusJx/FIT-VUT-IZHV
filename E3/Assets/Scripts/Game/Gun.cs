...
public class Gun : MonoBehaviour
{
    ...
    public void ShootGun(Transform director)
    {
        ...
        if (shotgun)
        {
            ...
            {
                float rnd = UnityEngine.Random.Range(-shotgunSpread / 2, shotgunSpread / 2);
                //Quaternion bulletRot = new Quaternion(director.rotation.x, director.rotation.y + rnd, director.rotation.z, director.rotation.w);
                Quaternion bulletRot = Quaternion.Euler(director.eulerAngles.x, director.eulerAngles.y + rnd, director.eulerAngles.z);
                SpawnBullet(director.position, bulletRot);
                StopFiring();
            }
        }
        else
