...
public class Enemy : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
         ...
         */
        mRigidBody = (GameManager.Instance.NearestPlayer(transform.position)).GetComponent<Rigidbody>();
        if(mRigidBody != null)
        {
            transform.position = Vector3.MoveTowards(transform.position, mRigidBody.position, speed * Time.deltaTime);
        }
    }
