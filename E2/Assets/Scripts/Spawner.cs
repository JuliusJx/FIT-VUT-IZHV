...
public class Spawner : MonoBehaviour
{
    ...
    /// <summary>
    /// Range in which obstacle's size is
    /// </summary>
    public float spawnSizeRange = .25f;
    ...
    public void SpawnObstacle()
    {
        ...
        float sizeModifier = Random.Range(-spawnSizeRange, spawnSizeRange);
        float size = spawnSize + sizeModifier;

        // Move it to the target location.
        var spawnDown = RandomBool();
        obstacle.transform.position += (Vector3)(spawnDown ?
            spawnOffset + (1.0f - size) / 2.0f :
            -spawnOffset - (1.0f - size) / 2.0f
        );
